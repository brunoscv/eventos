...
function recursiveMenuNav($menuList, $extra = ""){
	...
	foreach($menuList as $menu){
		...
		if( $hasSubmenu ){
			$str .= "<li class=\"{$class}\"><a href=\"{$url}\" class=\"{$classLink}\"><i class=\"menu-icon {$menu->icone}\"></i><span>" . $menu->descricao."</span><span class=\"arrow pull-right\"><i class=\"md md-add\"></i></span></a>";
			$str .= recursiveMenuChildNav($menu->filhos,'class="list-unstyled" style="display:none"');
		} else {
			$str .= "<li class=\"{$class}\"><a href=\"{$url}\" class=\"{$classLink}\"><i class=\"menu-icon {$menu->icone}\"></i><span>" . $menu->descricao."</span></a>";
		}
		$str .= "</li>";
	}
	$str .= "</ul></div>";
...
function recursiveMenuChildNav($menuList, $extra = "") {
	...
	foreach($menuList as $menu){
		...
		$str .= "<li><a href=\"{$url}\"></i>" . $menu->descricao."</a>";
